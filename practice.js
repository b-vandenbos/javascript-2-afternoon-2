...
function evenFinder(name) {
  let evenNums = [];
  for (var i = 0; i < name.length; i++) {
    if (name[i] % 2 === 0) {
      evenNums.push(name[i]);
    }
  } 
  return evenNums;
}
...
function removeEmp(arrayName, empName) {
  for (var i = 0; i < arrayName.length; i++) {
    if (arrayName[i].name === empName) {
...
function removeUser(array, useremail) {
